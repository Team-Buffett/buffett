...
LIQ_MIN_DEPTH_USDT = float(os.getenv("LIQ_MIN_DEPTH_USDT", "12000"))  # ETH 기준 최소 호가 유동성(추가 완화)
...
TURTLE_TREND_EMA = int(os.getenv("TURTLE_TREND_EMA", "120"))
FORCE_ENTRY_IDLE_STREAK = int(os.getenv("FORCE_ENTRY_IDLE_STREAK", "9999"))
FORCE_ENTRY_SIZE = float(os.getenv("FORCE_ENTRY_SIZE", "0.6"))
FORCE_ENTRY_LEVERAGE = int(os.getenv("FORCE_ENTRY_LEVERAGE", "3"))
...
def forced_entry_from_turtle(snapshot: Dict[str, Any]) -> Dict[str, Any]:
    """
    무포지션이 장기간 지속될 때(무거래 고착), 추세 필터 기반으로 소형 진입을 허용.
    - 터틀 신호가 없더라도 trend_up/down이 명확하면 작은 포지션으로 진입
    """
    tp = snapshot.get("turtle_pack", {}) or {}
    if not tp.get("ok", False):
        return {"direction": "NO_POSITION"}

    trend_up = bool(tp.get("trend_up", False))
    trend_down = bool(tp.get("trend_down", False))
    ...
    # 강제진입은 기대값을 해치지 않도록 RR 하한 준수
    tp_pct = clamp(sl_pct * max(rr_floor, 1.0), 0.003, 0.2)

    return {
        "direction": direction,
        "recommended_position_size": clamp(FORCE_ENTRY_SIZE, 0.5, 1.2),
        "recommended_leverage": int(clamp(FORCE_ENTRY_LEVERAGE, 1, MAX_LEVERAGE)),
        "stop_loss_percentage": sl_pct,
        "take_profit_percentage": tp_pct,
        "reasoning": f"[FORCED_ENTRY_IDLE] idle_streak>= {FORCE_ENTRY_IDLE_STREAK}; trend-filter continuation {direction}.",
    }
...
def main():
    ...
    while True:
        try:
            ...
            # 무포지션이 너무 길어질 때는 추세 필터 기반 소형 강제 진입으로 고착 해소
            if (
                decision.get("direction") == "NO_POSITION"
                and not fetch_current_position()
                and idle_no_pos_streak >= max(1, FORCE_ENTRY_IDLE_STREAK)
            ):
                forced = forced_entry_from_turtle(snapshot)
                if forced.get("direction") in ("LONG", "SHORT"):
                    decision = forced
                    log(f"[FORCE_ENTRY] idle={idle_no_pos_streak} → {decision['direction']} 소형 진입 시도")
